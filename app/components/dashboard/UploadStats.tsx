...
import { ChartBarIcon, CloudArrowUpIcon, DocumentTextIcon } from '@heroicons/react/24/outline';
import { useDocumentStore } from '../../store/documentStore';
import type { Document as MyDocument, DocumentStore } from '../../store/documentStore';

export default function UploadStats() {
  const documents = useDocumentStore((state: DocumentStore) => state.documents);

  const totalStorage = documents.reduce((acc: number, doc: MyDocument) => acc + doc.size, 0);
  ...
  const recentActivity = documents.filter((doc: MyDocument) => {
    const yesterday = new Date();
    yesterday.setDate(yesterday.getDate() - 1);
    return new Date(doc.uploadDate) > yesterday;
  }).length;
