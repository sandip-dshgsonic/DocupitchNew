...
export default function DocumentUpload() {
  ...
  // ✅ Fix: Explicitly type state for TypeScript
  const addDocument = useDocumentStore((state: DocumentStore) => state.addDocument);
    ...
    try {
      for (const file of files) {
        const doc = {
          ...
          type: file.type,
          uploadDate: new Date(),
          url: URL.createObjectURL(file), // Simulating a URL, replace with actual server upload
        };
