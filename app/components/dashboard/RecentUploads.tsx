...
// Define a custom interface for document objects
...
export default function RecentUploads() {
  const { documents, removeDocument } = useDocumentStore();
  // const recentDocuments: UploadedDocument[] = documents.slice(0, 5); // Show 5 most recent
  const recentDocuments: UploadedDocument[] = documents.slice(0, 5).map((doc) => ({
    id: doc.id,
    name: doc.name,
    uploadDate: doc.uploadDate.toISOString(), // convert Date to string
    size: doc.size,
    url: doc.url,
  }));
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 mb-8">
      ...
      {recentDocuments.length === 0 ? (
        ...
        <div className="space-y-2">
          {recentDocuments.map((doc: UploadedDocument) => (
            <div 
              key={doc.id}
              className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg"
            >
