...
export default function DataroomView({
  ...
  const handleSubmission = useCallback(async (): Promise<void> => {
    setIsLoading(true);
    const response = await fetch("/api/views-dataroom", {
      method: "POST",
      headers: {
  ...
  }, [submitted, isProtected, token, viewData.dataroomViewId,handleSubmission]);
...
// }, [documentData]);
  ...
  if (submitted && documentData) {
    return viewData.notionData?.recordMap ? (
      ...
      <div className="bg-gray-950" style={{height:'100%'}}>
        <PagesViewerNew
