...
export default function PagesViewer({
  ...
  }, [pageNumber, numPages, dataroomId, documentId, isPreview, linkId, versionNumber, viewId]);
  ...
  }, [dataroomId,router]); // Run once on mountdataroomId' and 'router
// }, []); // Run once on mountdataroomId' and 'router
  ...
  const handleScroll = useCallback(() => {
    ...
    if (
      ...
      trackPageView({ linkId, documentId, viewId, duration, pageNumber, versionNumber, dataroomId, setViewedPages, isPreview });
      setPageNumber(currentPage);
      pageNumberRef.current = currentPage;
      startTimeRef.current = Date.now();
      hasTrackedDownRef.current = true;
    ...
    if (
      ...
      trackPageView({ linkId, documentId, viewId, duration, pageNumber: pageNumber + 1, versionNumber, dataroomId, setViewedPages, isPreview });
      setPageNumber(currentPage);
      pageNumberRef.current = currentPage;
      startTimeRef.current = Date.now();
      hasTrackedUpRef.current = true;
                      ...
                      >
                        <img
                          ...
                          src={
                            loadedImages[index]
                              ? page.file
                              : "https://docupitch.com/_static/blank.gif"  // by aniket
