...
export default function EmailSection({
  ...
  useEffect(() => {
    ...
    const storedEmail = window.localStorage.getItem("docupitch.email");
    if (storedEmail) {
      setData((prevData) => ({
        ...prevData,
  ...
  const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    window.localStorage.setItem("docupitch.email", newEmail);
    // Update the state
    ...
    <div className="pb-5">
      <div className="relative space-y-2">
        ...
        <input
          ...
          autoComplete="email"
          autoFocus
          className="flex w-full px-2 rounded-md border-0 bg-black py-1.5 text-gray-500 shadow-sm ring-1 ring-inset ring-gray-600 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-gray-300 sm:text-sm sm:leading-6"
          style={{
            backgroundColor:
