...
export default function DomainSection({
  ...
  const handleDomainChange = (value: string) => {
    ...
    if (value === "add_domain" || value === "add_dataroom_domain") {
      ...
      setModalOpen(true);
      setData({ ...data, domain: "docupitch.com" });
      return;
    }
  ...
  useEffect(() => {
    if (domains && !editLink) {
      ...
      setData({
        ...data,
        domain: defaultDomain?.slug ?? "docupitch.com",
      });
    }
  }, [domains, editLink, data, setData]); // Add data and setData to the dependency array
// }, [domains, editLink]);

  const defaultDomain = editLink
    ? (data.domain ?? "docupitch.com")
    : (domains?.find((domain) => domain.isDefault)?.slug ?? "docupitch.com");
  ...
  return (
    <>
        ...
        >
          <SelectTrigger
            className={cn(
              ...
              data.domain && data.domain !== "docupitch.com"
                ? ""
          ...
          <SelectContent className="flex w-full rounded-md border border-input bg-white text-foreground placeholder-muted-foreground focus:border-muted-foreground focus:outline-none focus:ring-inset focus:ring-muted-foreground dark:border-gray-500 dark:bg-gray-800 focus:dark:bg-transparent sm:text-sm">
            <SelectItem value="docupitch.com" className="hover:bg-muted">
            docupitch.com
            </SelectItem>
        ...
        {data.domain && data.domain !== "docupitch.com" ? (
          <Input
            ...
            className={cn(
              ...
              data.domain && data.domain !== "docupitch.com" ? "flex" : "",
            )}
            placeholder="deck"
            onChange={(e) => {
      ...
      {data.domain && data.domain !== "docupitch.com" && !isDomainVerified ? (
