...
export default function Billing() {
  ...
  useEffect(() => {
    ...
    if (router.query.success) {
      ...
      }/billing/webhook`,
      {
        ...
        body: JSON.stringify({ planff: "ff" }), // Example payload
      },
    )
    ...
    //   const d =  prisma.team.update({
    //     where: { id: teamInfo?.currentTeam?.id },
    //     data: {
   ...
   console.log("-----73---billing tsx",tiers)
  ...
  // console.log("-----200---billing tsx",tiers)
                      ...
                      "tier-free"  ? (
                        tier.currentPlan ? (
                          <Button
                            ...
                            onClick={() => {
                              setClicked(true);
                              console.log('---469 billing button')
                              fetch(
                          ...
                          <Button
                            ...
                            onClick={() => {
                              setClicked(true);
                              console.log('---497 billing button')
                              fetch(
                                `/api/teams/${teamInfo?.currentTeam?.id}/billing/manage`,
                                {
                                  method: "POST",
...
<div className="p-6">
  {tier.id !== "tier-free" ? (
    <UpgradePlanModal
      clickedPlan={tier.name as "Pro" | "Business" | "Data Rooms"}
      trigger={"billing_page"}
    >
      <Button
        className="rounded-3xl"
        variant={tier.id === "tier-business" ? "orange" : "default"}
      >
        {tier.buttonText} b
      </Button>
    </UpgradePlanModal>
